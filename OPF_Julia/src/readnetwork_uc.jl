...
function readnetwork(testsystem)
    ...
    for i in 1:size(mpc["gen"],1)
        ...
        # ramp_agc = mpc["gen"][i,17]/mpc["baseMVA"] # (MW/min) -> (p.u./min)
        # ramp_10 = mpc["gen"][i,18]/mpc["baseMVA"] # (MW/10min) -> (p.u./10min)
        # ramp_30 = mpc["gen"][i,19]/mpc["baseMVA"] # (MW/30min) -> (p.u./30min)
        # ramp_q = mpc["gen"][i,20]/mpc["baseMVA"] # (MVAr/min) -> (p.u./min)
